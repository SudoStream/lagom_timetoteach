...
/**
  * This is an event sourced entity. It has a state, [[TimetoteachState]], which
  ...
  */
class TimetoteachEntity extends PersistentEntity {

  override type Command = TimetoteachCommand[_]
  override type Event = TimetoteachEvent
  override type State = TimetoteachState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: TimetoteachState = TimetoteachState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case TimetoteachState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        TimetoteachState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class TimetoteachState(message: String, timestamp: String)

object TimetoteachState {
  /**
    ...
    */
  implicit val format: Format[TimetoteachState] = Json.format
}

/**
  * This interface defines all the events that the TimetoteachEntity supports.
  */
sealed trait TimetoteachEvent extends AggregateEvent[TimetoteachEvent] {
  def aggregateTag = TimetoteachEvent.Tag
}

object TimetoteachEvent {
  val Tag = AggregateEventTag[TimetoteachEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends TimetoteachEvent
...
/**
  ...
  */
sealed trait TimetoteachCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends TimetoteachCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends TimetoteachCommand[String]
...
/**
  ...
  * application loader.
  */
object TimetoteachSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[TimetoteachState]
  )
}
